...
if __name__ == '__main__':
    ...
    cifar_10_train_dt = CIFAR10(r'~/Documents/data/cifar',  download=True, transform=ToTensor())
    ...
    epoch_restart = 0
    root = Path(f"./Models/run1/encoder/")
    ...
    for epoch in range(epoch_restart + 1, 31):
        batch = tqdm(cifar_10_train_l, total=len(cifar_10_train_dt) // batch_size)
        train_loss = []
        for x, target in batch:
            x = x.to(device) # image (64*3*32*32)

            optim.zero_grad()
            loss_optim.zero_grad()
            y, M = encoder(x) # y:64*64   M:64*128*26*26
